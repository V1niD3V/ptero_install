...
preflight(){
    output "Painel Pterodactyl -  Script"
    output "Copyright © 2020 Vinicius."
    output ""

    os_check

    if [ "$EUID" -ne 0 ]; then
        output "Entre via ROOT."
        exit 3
    fi
    ...
    if [ ${MACHINE_TYPE} == 'x86_64' ]; then
        output "À maquina é 64-bit. Iniciando."
        output ""
    else
        output "Entre em uma maquina 64 Bit (x86_64)."
        exit 4
    fi
...
install_options(){
    ...
    output "[5] Atualizar painel 0.7.x para 0.7.18."
    ...
    output "[9] Torne o Pterodactyl compatível com o aplicativo móvel (use isso apenas depois de instalar o painel - verifique https://pterodactyl.cloud para mais informações)."
    output "[10] Atualizar compatibilidade móvel."
    output "[11] Instale ou atualize para phpMyAdmin 5.0.2 (use somente depois de instalar o painel)."
    output "[12] Instalar um host de banco de dados independente (apenas para uso em instalações somente daemon)."
    ...
    case $choice in
        1) installoption=1
            output "Você selecionou apenas instalação de painel."
            ;;
        2) installoption=2
            output "Você selecionou apenas a instalação do daemon."
            ;;
        3) installoption=3
            output "Você selecionou a instalação do painel e do daemon."
            ;;
        4) installoption=4
            output "Você optou por instalar o servidor SFTP autônomo."
            ;;
        5) installoption=5
            output "Você optou por atualizar o painel."
            ;;
        6) installoption=6
            output "Você optou por atualizar o daemon."
            ;;
        7) installoption=7
            output "Você optou por atualizar o painel e o daemon."
            ;;
        8) installoption=8
        ...
        9) installoption=9
            ...
            ;;
        10) installoption=10
            output "Você optou por atualizar a compatibilidade do aplicativo móvel."
            ;;
        11) installoption=11
            output "Você optou por instalar ou atualizar o phpMyAdmin."
            ;;
        12) installoption=12
            output "Você optou por instalar um host de banco de dados."
            ;;
        13) installoption=13
            output "Você optou por alterar o tema do Pterodactyl."
            ;;
        14) installoption=14
            output "Você selecionou a redefinição de senha raiz do MariaDB."
            ;;
        15) installoption=15
...
theme_options() {
    output "Qual tema você quer instalar?"
    output "[1] Não quero."
    output "[2] Argon."
    output "[3] Kunufe."
    output ""
    ...
    case $choice in
        1 ) themeoption=1
            ...
            ;;
        2 ) themeoption=2
            output "Você selecionou o tema Argon! "
            output ""
            ;;
        3 ) themeoption=3
            output "Você selecionou o tema Kunufe."
            output ""
            ;;   
        * ) output "Você selecionou uma opção não valida."
            theme_options
    esac
}   
...
theme() {
    ...
    if [ "$themeoption" = "1" ]; then
        ...
        curl https://raw.githubusercontent.com/Vin-player/temas/master/install/argon.sh | sh
    elif [ "$themeoption" = "3" ]; then
        curl https://raw.githubusercontent.com/Vin-player/temas/master/install/kunufe.sh | sh
    elif [ "$themeoption" = "4" ]; then
        curl https://raw.githubusercontent.com/Vin-player/temas/master/install/dark.sh | sh
    fi
